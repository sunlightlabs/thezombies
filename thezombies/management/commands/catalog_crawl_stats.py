    ...
    def handle(self, *args, **options):
        ...
        for agency in agency_list:
            try:
                ...
                self.stdout.write(u"\nUpdated: {0}\n".format(date_str))
                self.stdout.write(u"| Errors | URLs inspected | URLs no content-type | FTP URLs | Possibly Invalid URLs |")
                self.stdout.write(u"| ------ | -------------- | -------------------- | -------- | --------------------- |")
                stats_row = u"| {errors:6,d} | {inspected: >14,d} | {typeless:20,d} | {ftp:8,d} | {invalid:21,d} |".format(
                            errors=audit.error_count(), inspected=num_urls_visited, typeless=typeless_http_inspections.count(),
                            ftp=ftp_inspections.count(), invalid=other_typeless_inspections.count())
